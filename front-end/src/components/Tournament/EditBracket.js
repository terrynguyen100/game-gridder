...
const EditBracket = ({ tournament, setTournament, numOfPlayers }) => {
  ...
  const updateScoresForMatch = async (
    match_id,
    winningPlayer,
    losingPlayer
  ) => {
              ...
              return (
                <>
                  <div class="names" style={{ gridArea: `name-${i + 1}` }}>
                    ...
                    >
                      {match?.players[0]?.player_name}
                    </h1>
                    <h1
                      className="edit-name"
                      style={{ gridArea: `name-${i + 1}` }}
                    >
                      {match?.players[1]?.player_name}
                    </h1>
                  </div>

                  <div
                    className="name-score-container"
                    style={{ gridArea: `game-${i + 1}` }}
                  >
