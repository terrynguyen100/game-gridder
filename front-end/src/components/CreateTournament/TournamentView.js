...
const TournamentView = () => {
  // ------------------- State & Context -------------------
  const [rounds, setRounds] = useState([]); // State for rounds
  const {
    tourMatches, setTourMatches, // Context values and setters
    tourParticipants, 
    usersIds, 
    tourPlayerNum, 
  } = useContext(CreateTournamentContext);

  // Updates the tourMatches state with match data
  const updateMatches = async () => {
    const matchesArray = [];

    // Loop through participants and create matches
  ...
  // Updates the rounds state based on participant count
  ...
  // Update matches and rounds when tourParticipants change
  useEffect(() => {
    updateMatches();
